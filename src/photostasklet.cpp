...
PhotosTasklet::PhotosTasklet(QObject *parent) : outstandingChecking(false)
{
    Q_UNUSED(parent);
}
...
QPixmap PhotosTasklet::squareQPixmapFromPath(QString path, qreal wR, qreal hR)
{
...
/*    int w = a.width();
    ...
    if (edge > h)
        edge = h; */
    QImage b = a.scaled(wR, hR, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation);
    QImage c = b.copy(0, 0, wR, hR);
...
void PhotosTasklet::checkThumbnailsGenerated()
{
    ...
    while (cnt-- > 0) {
        ...
        if (f.exists()) {
            qDebug() << "123456: w:" << AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().width() << ", h:" << AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().height();
            entry->thumbnail = new QPixmap(squareQPixmapFromPath(entry->thumbnailURI, AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().width(), AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().height()));
...
void PhotosTasklet::processSingleJob(Job& j)
{
    ...
    QFile f;
    f.setFileName(entry->thumbnailURI);
    qDebug() << "processSingleJob: row " << index.row() << "thumbnail:" << entry->thumbnailURI << "entry@" << (void *)entry << "exist?:" << f.exists();
    if (f.exists()) {
            qDebug() << "123456: w:" << AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().width() << ", h:" << AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().height();
        entry->thumbnail = new QPixmap(squareQPixmapFromPath(entry->thumbnailURI, AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().width(), AppWindow::instance()->photoList->cellCreator()->cellSize().toSize().height()));
        //entry->thumbnail = new QPixmap(squareQPixmapFromPath(entry->thumbnailURI, 112));
